...
export default function ModifyQuestion() {
  ...
  const handleQuestionOptionClick = (e) => {
    questionsArray.forEach((q) => {
      if (q.questionId === e) {
        setQuesId(q.questionId);
        setQuestion(q.question);

        if (typeof q.choice1 === "string") {
          if (JSON.parse(q.choice1).c) {
            setChoice1(JSON.parse(q.choice1).c);
            setChoice1Val(JSON.parse(q.choice1).v.toString());
          } else {
            setChoice1("");
            setChoice1Val("");
          }
          ...
          if (q.choice1.c) {
            setChoice1(q.choice1.c);
            setChoice1Val(q.choice1.v.toString());
          } else {
            setChoice1("");
            setChoice1Val("");
          }
        ...
        if (typeof q.choice2 === "string") {
          if (JSON.parse(q.choice2).c) {
            setChoice2(JSON.parse(q.choice2).c);
            setChoice2Val(JSON.parse(q.choice2).v.toString());
          } else {
            setChoice3("");
            setChoice3Val("");
          }
          ...
          if (q.choice2.c) {
            setChoice2(q.choice2.c);
            setChoice2Val(q.choice2.v.toString());
          } else {
            setChoice2("");
            setChoice2Val("");
          }
        ...
        if (typeof q.choice3 === "string") {
          if (JSON.parse(q.choice3).c) {
            setChoice3(JSON.parse(q.choice3).c);
            setChoice3Val(JSON.parse(q.choice3).v.toString());
          } else {
            setChoice3("");
            setChoice3Val("");
          }
          ...
          if (q.choice3.c) {
            setChoice3(q.choice3.c);
            setChoice3Val(q.choice3.v.toString());
          } else {
            setChoice3("");
            setChoice3Val("");
          }
        ...
        if (typeof q.choice4 === "string") {
          if (JSON.parse(q.choice4).c) {
            setChoice4(JSON.parse(q.choice4).c);
            setChoice4Val(JSON.parse(q.choice4).v.toString());
          } else {
            setChoice4("");
            setChoice4Val("");
          }
          ...
          if (q.choice4.c) {
            setChoice4(q.choice4.c);
            setChoice4Val(q.choice4.v.toString());
          } else {
            setChoice4("");
            setChoice4Val("");
          }
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMessage("Error: Cannot modify question:", error.message);
    }

    setSuccessMessage("Successfully modified question!");
    ...
    <div className="flex flex-col w-screen h-screen bg-stone-bg bg-cover">
      ...
      <div className="flex py-5 w-screen z-20 bg-stone-bg bg-cover place-items-center">
        <div className="flex relative place-items-center px-5 gap-5">
          ...
          />
          <span className="z-10 font-bold text-xl">Modify Question</span>
        </div>
      </div>
      {/* End of Header */}
      ...
      <div className="flex bg-white p-10">
        ...
        >
          <div className="flex flex-row w-full gap-5">
            ...
            <div className="flex grow relative min-w-[200px]">
              <select
                onChange={(e) => handleQuestionOptionClick(e.target.value)}
                className="peer h-20 w-full rounded-[7px] border-2 border-black-400 border-t-transparent bg-transparent px-3 py-2.5 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 empty:!bg-gray-900 focus:border-2 focus:border-gray-900 focus:border-t-transparent focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50"
              >
                {questionsArray &&
                  questionsArray.map((q) => {
                    return <option value={q.questionId}>{q.questionId}</option>;
                  })}
              </select>
              <label className="before:content[' '] after:content[' '] pointer-events-none absolute left-0 -top-1.5 flex h-full w-full select-none text-[11px] font-normal leading-tight text-blue-gray-400 transition-all before:pointer-events-none before:mt-[6.5px] before:mr-1 before:box-border before:block before:h-1.5 before:w-2.5 before:rounded-tl-md before:border-t before:border-l before:border-blue-gray-200 before:transition-all after:pointer-events-none after:mt-[6.5px] after:ml-1 after:box-border after:block after:h-1.5 after:w-2.5 after:flex-grow after:rounded-tr-md after:border-t after:border-r after:border-blue-gray-200 after:transition-all peer-placeholder-shown:text-sm peer-placeholder-shown:leading-[3.75] peer-placeholder-shown:text-blue-gray-500 peer-placeholder-shown:before:border-transparent peer-placeholder-shown:after:border-transparent peer-focus:text-[11px] peer-focus:leading-tight peer-focus:text-gray-900 peer-focus:before:border-t-2 peer-focus:before:border-l-2 peer-focus:before:border-gray-900 peer-focus:after:border-t-2 peer-focus:after:border-r-2 peer-focus:after:border-gray-900 peer-disabled:text-transparent peer-disabled:before:border-transparent peer-disabled:after:border-transparent peer-disabled:peer-placeholder-shown:text-blue-gray-500">
                Select Level
